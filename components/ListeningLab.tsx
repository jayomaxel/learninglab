...
import { db } from '../services/db'; // Updated import
...
const ListeningLab: React.FC<ListeningLabProps> = ({ language, onSaveWord }) => {
         ...
         if (seg && video.currentTime >= seg.end) {
           ...
           if (seg) {
               ...
               if (nextState === 'LOOP') {
                   ...
                   if (video.currentTime >= anchor || video.currentTime < loopStart) {
  ...
  const handleWordChange = (index: number, value: string) => {
    ...
    if (isMatch) {
      if (wordResults[index] !== 'correct') {
          correctWordsCountRef.current += 1;
          totalWordsCountRef.current += 1;
      }
    ...
    <section className="space-y-6 relative">
      {showWarmup && analysisResult && (
        <DifficultyWarmup 
            ...
            onCancel={() => { setShowWarmup(false); setMediaUrl(null); setSegments([]); }}
        />
      )}
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-4 shadow-sm">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button onClick={() => setInputMode('PROMPT')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${inputMode === 'PROMPT' ? 'bg-white text-green-600' : 'text-slate-400'}`}>自定义主题</button>
            <button onClick={() => setInputMode('URL')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${inputMode === 'URL' ? 'bg-white text-green-600' : 'text-slate-400'}`}>YouTube / 链接</button>
          </div>
          <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
             <button onClick={() => setFlowMode(!flowMode)} className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 border ${flowMode ? 'bg-indigo-500 text-white border-transparent ring-2 ring-indigo-200' : 'bg-white text-indigo-500 border-indigo-200 hover:bg-indigo-50'}`} title="开启后，语速将随打字速度自动调整，停顿时自动循环">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                Flow Sync {flowMode ? 'ON' : 'OFF'}
             </button>
             <button onClick={playFullAudio} disabled={!mediaUrl || showWarmup} className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 border ${isFullPlaying ? 'bg-green-400 text-white border-transparent' : 'bg-slate-900 text-white border-transparent hover:bg-slate-800 disabled:opacity-50'}`}>
        ...
        <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-xl border border-slate-200 focus-within:border-green-400 transition-all">
          ...
          />
          <button onClick={handleAISubmit} disabled={loading} className="bg-green-400 text-white px-6 py-2.5 rounded-xl text-xs font-bold hover:bg-green-500 disabled:opacity-50 transition-colors">
        ...
        {flowMode && isFullPlaying && (
            <div className="flex justify-center animate-in fade-in duration-300">
                <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold shadow-sm transition-colors duration-300 ${flowState === 'FLOW' ? 'bg-green-100 text-green-700' : flowState === 'SLOW' ? 'bg-yellow-100 text-yellow-700' : 'bg-indigo-100 text-indigo-700'}`}>
        ...
        <div className="lg:col-span-7 space-y-6">
            ...
            {mediaUrl ? (
              ...
              <div className="text-center p-8 bg-white w-full h-full flex flex-col items-center justify-center space-y-4">
                ...
                <div className="flex gap-2">
                   <button onClick={() => fileInputRef.current?.click()} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition-all border border-slate-200">选择媒体文件</button>
                </div>
              </div>
            )}
          ...
          <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200">
             ...
             {segments.length > 0 && (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full">已加载字幕</span>
                </div>
             )}
          </div>
          ...
          {currentSegment && (
            <div className="bg-white p-6 rounded-2xl border border-slate-200 relative overflow-hidden shadow-sm">
              ...
              <p className="text-lg text-slate-700 font-bold mb-2 leading-relaxed">{currentSegment.translation || '（暂无翻译）'}</p>
              {showHint && (
                <div className="bg-slate-50 p-3 rounded-xl mt-2 border border-slate-100">
                   <p className="text-sm text-slate-400 italic font-mono mb-2">
                     {segmentWords.map((word, i) => (
                       <span key={i} onClick={() => onSaveWord(word, currentSegment.text)} className="hover:text-green-600 hover:bg-green-100 cursor-pointer rounded px-0.5 transition-all" title="点击添加到生词本">{word}{' '}</span>
                     ))}
                   </p>
                </div>
              )}
           ...
           {segments.length > 0 ? (
            <div className="bg-white rounded-3xl p-8 border border-slate-200 min-h-[400px] flex flex-col shadow-sm">
                  ...
                  {segmentWords.map((word, idx) => (
                    <div key={idx} className="relative">
                      <input
                        ...
                        className={`text-center py-2.5 px-1 text-lg font-mono font-bold rounded-xl border-2 transition-all outline-none ${wordResults[idx] === 'correct' ? 'bg-green-50 border-green-400 text-green-700' : wordResults[idx] === 'wrong' ? 'bg-red-50 border-red-400 text-red-700' : 'bg-slate-50 border-slate-100 focus:border-green-300 focus:bg-white'}`}
                        autoComplete="off"
                        autoCapitalize="off"
                        spellCheck="false"
                      />
              ...
              <div className="mt-8 grid grid-cols-2 gap-4">
                <button onClick={playCurrentSegment} disabled={showWarmup} className="py-4 bg-green-400 hover:bg-green-500 text-white rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 disabled:opacity-50"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /></svg>重听本句</button>
                <button onClick={() => { if (currentIndex < segments.length - 1) setCurrentIndex(prev => prev + 1); }} disabled={showWarmup} className="py-4 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-2xl font-bold transition-all active:scale-95 disabled:opacity-50">跳过句子 (Ctrl+→)</button>
              </div>
            </div>
          ) : (
            <div className="flex-1 bg-slate-50 rounded-3xl border-dashed border-2 border-slate-200 flex flex-col items-center justify-center p-12 text-center">
              ...
              <p className="text-slate-400 font-bold leading-relaxed max-w-xs mb-4">{mediaUrl ? "已加载媒体文件。<br/>请点击左侧“上传字幕”以开始练习。" : "听力实验室已切换为精准断句模式。<br/>点击“生成实验室”或上传本地文件。"}</p>
            </div>
          )}
        </div>
      </div>
