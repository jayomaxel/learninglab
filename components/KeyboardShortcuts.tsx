  ...
  const groups = [
    ...
    },
    {
      ...
      keys: [
        ...
        { key: 'Esc', desc: '显示或隐藏原文' },
      ],
    },
    {
      title: '极速阅读',
      keys: [
        { key: 'Space', desc: '开始或暂停播放' },
        { key: 'Ctrl + ←/→', desc: '调整每分钟词数速度' },
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-[32px] p-8 border border-slate-200 shadow-2xl">
              ...
              <div className="space-y-2">
                {group.keys.map((item) => (
                  <div key={item.key} className="flex items-center justify-between">
                    ...
                    <kbd className="px-2 py-1 bg-slate-100 border border-slate-200 rounded-lg text-[10px] font-mono font-bold text-slate-500">
                      {item.key}
                    </kbd>
                  </div>
                ))}
        ...
        <button onClick={onClose} className="w-full mt-8 py-3 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-sm transition-all">
          知道了
        </button>
      </div>
    </div>
