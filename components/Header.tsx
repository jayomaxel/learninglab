...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 p-2 sm:p-4">
      <div className="max-w-6xl mx-auto flex items-center justify-between bg-white rounded-2xl p-2 px-3 sm:px-6 border border-slate-200">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-green-400 rounded-xl flex items-center justify-center">
            <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <h1 className="text-lg sm:text-xl font-bold text-slate-800 tracking-tight hidden xs:block">
            LinguistFlow
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
            ...
            {(['EN', 'FR', 'KR'] as Language[]).map((lang) => (
              <button
                ...
                className={`w-8 sm:w-10 py-1 sm:py-1.5 rounded text-[10px] sm:text-xs font-bold transition-all ${
                  language === lang ? 'bg-white text-green-600' : 'text-slate-400 hover:text-slate-600'
                }`}
              >
                {lang}
              </button>
          ...
          <button 
            ...
            className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-800 transition-all border border-slate-200"
            title="设置"
          >
