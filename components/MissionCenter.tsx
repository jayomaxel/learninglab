...
interface MissionCenterProps {
  user: User;
  language: Language;
  onStartTask: (tab: AppState['currentTab']) => void;
  onStartReview: () => void;
  reviewStats: {
...
const MissionCenter: React.FC<MissionCenterProps> = ({
  user,
  language,
  onStartTask,
  onStartReview,
  ...
  const mission = user.missionStatus?.[`${dateKey}_${user.id}`] || {
    wordsCount: 0,
    listeningDone: false,
    readingDone: false,
  };
  ...
  const handleTaskOneClick = () => {
    if (taskOne.target === 'VOCAB') {
      onStartReview();
      return;
    }
    onStartTask(taskOne.target);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div onClick={handleTaskOneClick} className="bg-white p-6 rounded-xl border border-green-100 hover:bg-green-50 cursor-pointer">
          <div className="w-10 h-10 rounded-lg bg-green-50 text-green-600 flex items-center justify-center mb-4 border border-green-100">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={taskOne.icon} />
            </svg>
          </div>
          ...
          {taskOne.target === 'VOCAB' ? (
            <div className="grid grid-cols-3 gap-2 border-t border-green-50 pt-3">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase">需背</p>
                <p className="text-sm font-black text-green-700">{reviewStats.needReviewCount}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase">已背</p>
                <p className="text-sm font-black text-slate-700">{reviewStats.reviewedTodayCount}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase">总计</p>
                <p className="text-sm font-black text-slate-700">{reviewStats.totalDeckCount}</p>
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-between border-t border-green-50 pt-3">
              ...
              <div className="w-12 h-1 bg-green-50 rounded-full overflow-hidden border border-green-100">
                <div className="h-full bg-green-500" style={{ width: `${Math.min(100, (mission.wordsCount / 5) * 100)}%` }} />
              </div>
            </div>
          )}
        </div>
