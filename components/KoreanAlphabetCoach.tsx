...
const CONSONANTS = [
  ...
  { char: 'ㅇ', pinyin: '', idx: 11 },
  { char: 'ㅈ', pinyin: 'j', idx: 12 },
  { char: 'ㅊ', pinyin: 'ch', idx: 14 },
    ...
    <div className="flex flex-col gap-6">
      {pendingVerification && (
        <div className="bg-white p-4 rounded-2xl border-2 border-green-500 flex items-center justify-between shadow-lg sticky top-24 z-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-slate-800">确认智能模拟发音</p>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">确认后将永久保存到本地缓存</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setPendingVerification(null)} className="px-4 py-2 text-xs font-bold text-slate-400">
              取消
            </button>
            <button onClick={handleSaveCache} className="px-6 py-2 bg-green-500 text-white text-xs font-bold rounded-xl">
              保存到本地
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-[#e2f2e5] shadow-sm overflow-hidden">
        <div className="p-8 text-center border-b border-[#e2f2e5] bg-[#fafff5]">
          ...
          <p className="text-slate-400 text-[10px] mt-2 font-bold uppercase tracking-[0.2em]">智能母语模拟 · 本地缓存</p>
        </div>
          ...
          <thead>
            <tr className="bg-[#fafff5]">
              <th className="border border-[#e2f2e5] p-2 relative w-16 h-16">
                ...
                <div className="w-full h-px bg-[#e2f2e5] absolute top-1/2 left-0 -rotate-[45deg]" />
              </th>
              {VOWELS.map((v) => (
