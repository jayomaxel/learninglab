...
import { Language, VocabularyItem, StudyLog, User } from '../types';
import { calculateLexicalPower, calculateStreak } from '../services/stats';
...
const Dashboard: React.FC<DashboardProps> = ({ language, vocabulary, onStartReview, user }) => {
  ...
  useEffect(() => {
    db.getLogs(language, user.id).then(setLogs);
  }, [language, user.id]);
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-8 border border-green-200">
          ...
          <div className="text-5xl font-black text-slate-800 leading-none mb-4">
            {stats.lexicalPower} <span className="text-green-500 text-3xl">LXP</span>
          </div>
          <div className="inline-block bg-green-50 text-green-600 px-3 py-1 rounded-full text-xs font-bold border border-green-100">
            连续学习 {stats.streak} 天
          </div>
        </div>
            ...
            {(['EN', 'FR', 'KR'] as Language[]).map((lang) => (
              <div key={lang} className="space-y-1">
                <div className="flex justify-between text-[10px] font-bold text-slate-500">
                  <span>{lang}</span>
                  <span>{user.levels[lang]}</span>
                </div>
                <div className="h-2 bg-green-50 rounded-full border border-green-100 overflow-hidden">
                  <div
                    className="h-full bg-green-500"
                    style={{ width: `${(['A0', 'A1', 'A2', 'B1', 'B2', 'C1', 'C2'].indexOf(user.levels[lang]) + 1) * 14}%` }}
                  />
                </div>
              </div>
            ))}
      ...
      <div className="bg-green-600 rounded-3xl p-8 text-white flex flex-col sm:flex-row items-center justify-between gap-6 border border-green-700">
        ...
        <button onClick={onStartReview} className="px-8 py-3 bg-white text-green-600 rounded-xl font-bold border border-green-100">
          开始今日复习
        </button>
      </div>
    </div>
