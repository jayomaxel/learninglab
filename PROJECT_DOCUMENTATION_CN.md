...
1. Cuckoo/Bloom 过滤器
   - 现状：实现了过滤器和解析产物返回，但未接入在线查询主路径。
2. 形态学规则（`linguistics.ts`）
   - 现状：词形还原与韩语结构分析函数存在，当前 UI 主流程未调用。
3. Dashboard 高级统计函数
...
- 去重思路：Worker 内 Cuckoo 过滤器（当前主查链路未闭环）
...
4. 问：字典去重为什么同时有 Bloom 和 Cuckoo？
   - 答：代码存在演进轨迹，目前主用 Cuckoo 思路；Bloom 实现保留但未作为主路径。
